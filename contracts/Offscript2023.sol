...
contract Offscript2023 is ERC721, Ownable, IOffscript2023 {
    ...
    constructor(
        ...
        require(publicNames.length == _remainingPublicSupply, "different amount of names");
    ...
    function mintPublic() public payable {
        ...
        uint256 newItemId = uint256(totalPublicSupply - remainingPublicSupply + 1);

        uint8 random =
            uint8(uint256(keccak256(abi.encodePacked(msg.sender, tx.origin, block.difficulty, block.timestamp))));
    ...
    function mintPrivate(address[] calldata _addresses, uint8[] calldata _discounts, string[] calldata _names)
        external
        onlyOwner
    {
        ...
        uint256 nextId = uint256(totalPrivateSupply + totalPublicSupply - remainingPrivateSupply + 1);

        remainingPrivateSupply -= length;

        for (uint8 i = 0; i < length; i++) {
            _mintWithMetadata(_addresses[i], nextId + i, _discounts[i], _names[i]);
        }
    }
    ...
    // Internal API
    //
    function _mintWithMetadata(address _owner, uint256 _id, uint8 _discount, string memory _name) internal {
        metadata[_id] = Metadata(_discount, _name);
        _safeMint(_owner, _id);
    }

    function calculateDiscount(uint8 _random) internal returns (uint8 discount) {
